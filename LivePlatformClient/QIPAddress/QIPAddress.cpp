...
QIPAddress::QIPAddress(QWidget* parent) : QWidget(parent)
{
    ...
    verticalLayout->addWidget(frameIP);

    //将控件按照横向布局排列
    QHBoxLayout* layout = new QHBoxLayout(frameIP);
    layout->setMargin(0);
    layout->setSpacing(0);
...
{
    //先检测IP地址是否合法
    QRegExp regExp(R"(((2(5[0-5]|[0-4]\d))|[0-1]?\d{1,2})(\.((2(5[0-5]|[0-4]\d))|[0-1]?\d{1,2})){3})");
...
QString QIPAddress::getBgColor() const
{
    return bgColor;
}
...
QString QIPAddress::getBorderColor() const
{
    return borderColor;
}
...
int QIPAddress::getBorderRadius() const
{
    return borderRadius;
}

void QIPAddress::applyStyleSheet()
{
    QStringList qss;
    ...
    frameIP->setStyleSheet(qss.join(""));
}
